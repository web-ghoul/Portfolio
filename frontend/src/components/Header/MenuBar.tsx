    ...
    return (
        <nav className='hidden max-mobile:grid grid-cols-4 justify-stretch items-center gap-2 sticky py-3 bg-background border-t border-primary_border z-1000 -bottom-1 contain'>
            <MenuItem link={`${process.env.NEXT_PUBLIC_HOME_ROUTE}`} title={"Home"} >
                <HomeIcon className={`h-auto m-auto ${pathname === `${process.env.NEXT_PUBLIC_HOME_ROUTE}` ? "text-white w-6 " : "text-gray-400 w-5 "}`} />
            </MenuItem>
            <MenuItem link={`${process.env.NEXT_PUBLIC_WORKS_ROUTE}`} title={"Works"} >
                <FolderIcon className={`h-auto m-auto ${pathname === `${process.env.NEXT_PUBLIC_WORKS_ROUTE}` ? "text-white w-6 " : "text-gray-400 w-5 "}`} />
            </MenuItem>
            <MenuItem link={`${process.env.NEXT_PUBLIC_ABOUT_ROUTE}`} title={"About"} >
                <LogoIcon theme={pathname === `${process.env.NEXT_PUBLIC_ABOUT_ROUTE}` ? "light" : "dark"} className={`h-auto m-auto ${pathname === `${process.env.NEXT_PUBLIC_ABOUT_ROUTE}` ? "text-white w-6 " : "text-gray-400 w-5 "}`} />
            </MenuItem>
            <MenuItem link={`${process.env.NEXT_PUBLIC_CONTACT_ROUTE}`} title={"Contact"} >
                <PhoneIcon className={`h-auto m-auto ${pathname === `${process.env.NEXT_PUBLIC_CONTACT_ROUTE}` ? "text-white w-6 " : "text-gray-400 w-5 "}`} />
            </MenuItem>
        </nav>
    )
}
...
const MenuItem = ({ link, title, children }: { link: string; title: string; children: ReactNode }) => {
    return (
        <Link href={link} className="grid justify-center items-center gap-1 max-small:gap-0.5">
            {children}
            <p>{title}</p>
        </Link>
    )
